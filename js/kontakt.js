...
function paddingElementRight() {
    let elementH1Padding = document.getElementById('block__h1-pading').offsetLeft;
    const elementInfo = document.getElementById('container__info-padding');

    elementInfo.style.paddingLeft = `${elementH1Padding}px`;
};
paddingElementRight();
...
(function scrollBlockHiden() {
    const btnElem = document.querySelector('.btn__scroll-top');
    window.addEventListener("scroll", function() { 
        const conteinerFooter = document.querySelector('.container__footer').offsetTop - 800;
        let scrollPositionY = window.pageYOffset || document.documentElement.scrollTop; 
